...
namespace FtpClient
{
    public partial class Main : Form
    {
        ...
        public Main()
        {
            ...
            catch (ComponentPro.Licensing.Ftp.UltimateLicenseException exc)
            {
        ...
        async void FtpConnect(Ftp ftp)
        {
            ftp.Passive = _loginSettings.PasvMode;
            ...
            FtpApplySettings(ftp);

            try
            {
                // Asynchronously connect to the server.
                await ftp.ConnectAsync(_loginSettings.ServerName, _loginSettings.ServerPort, _loginSettings.SecurityMode); // FTP connection
            }
            catch (Exception exc)
            {
                Util.ShowError(exc);
        ...
        async void FtpAuthenticate(Ftp ftp)
        {
            try
            {
                // Asynchronously login.
                await ftp.AuthenticateAsync(_loginSettings.UserName, _loginSettings.Password);
            }
            catch (Exception ex)
            {
                Util.ShowError(ex);
        ...
        void FtpApplySettings(Ftp ftp)
        {
            ftp.UploadBufferSize = _ftpSettings.UploadBufferSize * 1024;
            ftp.KeepAliveDuringTransferInterval = _ftpSettings.KeepAlive;
            ftp.TransferMode = _ftpSettings.Compress ? FtpTransferMode.ZlibCompressed : FtpTransferMode.Stream;
            ftp.SendAbortCommand = _ftpSettings.SendAbor;
            ftp.SendTelnetInterruptSignal = _ftpSettings.SendSignals;
            ftp.ChangeDirectoryBeforeFileOperation = _ftpSettings.ChangeDirBeforeTransfer;
            ftp.ChangeDirectoryBeforeListing = _ftpSettings.ChangeDirBeforeListing;
            ftp.SmartPathResolving = _ftpSettings.SmartPath ? SmartPathResolvingMode.Default : SmartPathResolvingMode.None;
        }

        void FtpAddItemPermissions(ListViewItem item, AbstractFileInfo f)
        {
            if (((FtpFileInfo)f).System == FtpFileOs.Unix)
                item.SubItems.Add(ToPermissions(((FtpFileInfo)f).Permissions));
            else
                item.SubItems.Add("-");
        }

        async void FtpShowItemProperties(ListViewItem item)
        {
            ...
            if (item.ImageIndex == _folderIconIndex && dlg.Recursive != RecursionMode.None)
            {
                EnableProgress(true);
                ...
                try
                {
                    ...
                    await client.SetMultipleFilesPermissionsAsync(dlg.FileName, dlg.Permissions, dlg.Recursive, null);
                }
                catch (Exception ex)
                {
            ...
            else
            {
                try
                {
                    ...
                    client.SetFilePermissions(dlg.FileName, dlg.Permissions);
                    RefreshRemoteList();
                }
                catch (Exception exc)
                {
        ...
        private void client_CertificateRequired(object sender, ComponentPro.Security.CertificateRequiredEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_loginSettings.Certificate))
            {
                // Load Certificate.
                ...
                {
                    X509Certificate2 clientCert = new X509Certificate2(_loginSettings.Certificate, passdlg.Password);
        ...
        void ListDefaultDirectory()
        {
            ...
            try
            {
                if (_loginSettings.ClearCommandChannel && _loginSettings.SecurityMode != SecurityMode.None)
        ...
        async void RefreshRemoteList()
        {
            ...
            try
            {
                ...
                ShowRemoteFiles(await client.ListDirectoryAsync());
            }
            catch (Exception exc)
            {
        ...
        /// Deletes selected items.
        /// </summary>
        async void DoRemoteDelete(List<AbstractFileInfo> list, SearchCondition condition)
        {
            if (!Ready)
                return;
            ...
            try
            {
                await client.DeleteFilesAsync(list, RecursionMode.Recursive, condition == null, condition);
            }
            catch (Exception ex)
            {
                if (!HandleException(ex))
        ...
        const string TempDirName = "\\FtpClientDemo";
        ...
        async void DoRemoteView(ListViewItem item)
        {
            // Busy?
            if (!Ready)
                return;
            ...
            if (item.ImageIndex != UpFolderImageIndex && item.ImageIndex != FolderLinkImageIndex && item.ImageIndex != _folderIconIndex) // Not a folder or a symlink
            {
                ...
                try
                {
                    // Asynchronously download the remote file.    
                    await client.DownloadFileAsync(item.Text, tempFile, tempFile);
                }
                catch (Exception exc)
                {
                    if (!HandleException(exc))
                    ...
                    FileSystemException fe = exc as FileSystemException;
        ...
        async void DoSynchronize()
        {
                        ...
                        MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    await client.SynchronizeAsync(txtRemoteDir.Text, txtLocalDir.Text, dlg.RemoteIsMaster, opt);
                }
                catch (Exception ex)
                {
                    if (!HandleException(ex))
        ...
        async void DoLocalUploadAdvanced(IList items)
        {
			...
			{                
				...
				await client.UploadFilesAsync(txtLocalDir.Text, files, _currentDirectory, opt);
			}
			catch (Exception exc)
			{
				if (!HandleException(exc))
        ...
        /// </summary>
        /// <param name="item">The selected item.</param>
        async void DoLocalUploadUnique(ListViewItem item)
        {
        ...
        public async void DoRemoteDownloadAdvanced(IList items)
        {
            ...
            {                
                ...
                await client.DownloadFilesAsync(_currentDirectory, fileList, txtLocalDir.Text, opt);
            }
            catch (Exception exc)
            {
                if (!HandleException(exc))
                ...
                case Keys.F9:
                    ...
                    if (e.Alt && e.Control && ftp != null)
                    {
                        if (ftp.TransferMode == FtpTransferMode.ZlibCompressed)
                        {
                            ...
                            MessageBox.Show("ModeZ deactivated", "Ftp Client Demo");
                        }
                        else
                        {
                            ftp.TransferMode = FtpTransferMode.ZlibCompressed;
                            MessageBox.Show("ModeZ activated", "Ftp Client Demo");
                        }
                    }
                    break;
        ...
        private void mnuPopExecuteCommand_Click(object sender, EventArgs e)
        {
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    client.SendCommand(dlg.Command);
                    client.ReadResponse();
                    _lastCommand = dlg.Command;

                    if (_lastCommand.StartsWith("cwd ", StringComparison.InvariantCultureIgnoreCase))
                    {
                        _currentDirectory = client.GetCurrentDirectory();
                        txtRemoteDir.Text = _currentDirectory;
                    }
