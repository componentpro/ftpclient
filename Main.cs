...
namespace FtpClient
{
    public partial class Main : Form
    {
        ...
        void FtpConnect(Ftp ftp)
        {
            ...
            ftp.ConnectAsync(_loginSettings.ServerName, _loginSettings.ServerPort, _loginSettings.SecurityMode); // FTP connection
        }

        void FtpAuthenticate(Ftp ftp)
        {
            // Asynchronously login. AuthenticateCompleted event will be raised when the operation completes.
            ftp.AuthenticateAsync(_loginSettings.UserName, _loginSettings.Password);
        }
        ...
        void FtpShowItemProperties(ListViewItem item)
        {
            ...
            if (item.ImageIndex == _folderIconIndex && dlg.Recursive != RecursionMode.None)
            {
                ...
                // Asynchronously set permissions of multiple files. SetMultipleFilesPermissionsCompleted event is fired when the operation completes.
                client.SetMultipleFilesPermissionsAsync(dlg.FileName, dlg.Permissions, dlg.Recursive, null);
            }
            else
            {
                try
        ...
        /// </summary>
        /// <param name="item">The selected item.</param>
        void DoLocalUploadUnique(ListViewItem item)
        {
            string localFile = GetItemFullName(item);

            EnableProgress(true);

            // Asynchronously upload unique the selected file. UploadUniqueFileCompleted event will be fired when the operation completes.
            client.UploadUniqueFileAsync(localFile, GetFullPath(System.IO.Path.GetFileName(localFile)));
        }
